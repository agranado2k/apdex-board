...
//time complexity O(N)
window.NewRelicChallenge.Host.prototype.addApp = function(app) {
  var pos = this.findPosition(app, this.apps); //O(N)
  this.apps.splice(pos,0,app); //O(logN)
}

//time complexity O(logN)
window.NewRelicChallenge.Host.prototype.findPosition = function(app, appList) {
  var startIndex = 0;
...
//O(N)
window.NewRelicChallenge.Host.prototype.removeApp = function(app) {
  var appIndex = this.apps.indexOf(app);
  this.apps.splice(appIndex,1);
};

window.NewRelicChallenge.Host.prototype.viewCreateHostTag = function(){
